...
export const PUT = async (req) => {
  try {
    ...
    if (status) {
      switch (status) {
        ...
        case 202:
          ...
          const pricePerDay202 =
            carDoc && carDoc.calculatePrice
              ? carDoc.calculatePrice(rentalDays202)
              : 0;
          const totalPrice202 = pricePerDay202 * rentalDays202;
    ...
    const rentalDays = Math.ceil((end - start) / (1000 * 60 * 60 * 24));
    const pricePerDay =
      carDoc && carDoc.calculatePrice ? carDoc.calculatePrice(rentalDays) : 0;
    const totalPrice = pricePerDay * rentalDays;
